...
Omnidist builds the same `./cmd/jevlint` executable for Linux amd64/arm64,
macOS amd64/arm64, and Windows amd64. The npm meta packages `jevlint` and
`@diffpal/jevlint` both point to one set of five `@diffpal/jevlint-*`
platform packages. The configured version comes from `OMNIDIST_VERSION` and
is embedded in `jevlint version`. The first public release is planned as
`v0.1.0` under MIT with Alexey Samoylov as copyright holder. Registry package
publication has not yet been verified; the example below stages locally.

From the repository root, with Go, Node, and npm available:

```bash
export OMNIDIST_VERSION=0.1.0-dev.1
...
Staging does not establish availability of `jevlint` or `@diffpal/jevlint` in
...
The [v0.1.0 release runbook](release-v0.1.0.md) records the exact package and
