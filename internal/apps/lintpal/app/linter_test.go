...
import (
	...
	reportpkg "github.com/diffpal/lintpal/internal/apps/lintpal/report"
...
func (p *reorderProvider) Evaluate(ctx context.Context, request jev.Request) (jev.Response, error) {
	...
	if strings.HasSuffix(id, "/correctness/ignored-error.md") {
		select {
		case <-ctx.Done():
...
func TestLintNoApplicableRules(t *testing.T) {
	...
	pack, err := rules.Load(strings.NewReader("schema: lintpal.rules.v1\nrules:\n  - id: docs-only\n    type: noul\n    instructions: Is this wrong?\n    threshold: 0.9\n    severity: high\n    title: Docs\n    message: Docs issue.\n    paths: ['*.md']\n"))
	if err != nil {
		t.Fatal(err)
	}
	provider := &lintProvider{}
