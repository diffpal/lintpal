...
func TestCorpusRejectsMalformedAndOversized(t *testing.T) {
	for _, input := range []string{"", `{"schema_version":"jevlint.eval.corpus.v1","cases":[]}`,
		`{"schema_version":"jevlint.eval.corpus.v1","cases":[{"id":"x"}]}`,
		`{"schema_version":"jevlint.eval.corpus.v1","cases":[],"unexpected":true}`,
