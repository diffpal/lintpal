...
func NewRoot(lint LintFunc, version string) *cobra.Command {
	root := &cobra.Command{Use: "jevlint", Short: "Lint committed Git changes with typed decisions", SilenceUsage: true, SilenceErrors: true}
	...
	root.AddCommand(&cobra.Command{Use: "version", Short: "Print the jevlint version", Args: func(_ *cobra.Command, args []string) error {
		if len(args) > 0 {
			return ErrInvalidOptions
		}
		return nil
	}, RunE: func(cmd *cobra.Command, _ []string) error {
		_, err := fmt.Fprintf(cmd.OutOrStdout(), "jevlint %s\n", version)
		return err
	}})
