...
func TestProcessExitAndStreams(t *testing.T) {
	...
	if code != 0 || stderr != "" || !strings.Contains(stdout, `"schema_version": "jevlint.report.v1"`) || !strings.Contains(stdout, `"diagnostics": [`) {
	...
	if humanCode != 0 || humanErr != "" || !strings.HasPrefix(humanOut, "jevlint jevlint.report.v1 ") || !strings.Contains(humanOut, "high RIGHT") || !strings.Contains(humanOut, "stats work_items=") {
	...
	if err := os.WriteFile(badRules, []byte("schema: jevlint.rules.v1\nprovider: https://attacker.invalid\nrules: []\n"), 0600); err != nil {
		t.Fatal(err)
	}
	before = calls.Load()
...
func TestOptionalLiveEvalUsesPinnedLocalProvider(t *testing.T) {
	...
	command.Env = append(testEnv(), "JEVLINT_EVAL_BIN="+binary, "JEVLINT_EVAL_PROVIDER=custom",
		"JEVLINT_EVAL_BASE_URL="+server.URL, "JEVLINT_EVAL_MODEL=jev-1.13.0", "JEVLINT_EVAL_OUTPUT="+output,
		"JEVLINT_EVAL_INPUT_USD_PER_MILLION=1", "JEVLINT_EVAL_OUTPUT_USD_PER_MILLION=2",
		"JEVLINT_RULES=/missing-rules.yaml", "JEVLINT_OUT=/unexpected-report.json")
	...
	if calls.Load() != 16 || manifest.SchemaVersion != "jevlint.eval.live.v1" ||
...
func testEnv() []string {
	...
	for _, entry := range os.Environ() {
		...
		if strings.HasPrefix(key, "JEVLINT_") || key == "TYPESAFE_API_KEY" || key == "OPENROUTER_API_KEY" {
			continue
		}
		env = append(env, entry)
	}
	return append(env, "JEVLINT_TOKEN=secret-sentinel", "JEVLINT_PROVIDER=jev", "JEVLINT_BASE=invalid-env-base")
}
